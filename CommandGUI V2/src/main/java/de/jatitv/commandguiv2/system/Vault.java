    ...
    public static void loadVault() throws InterruptedException {
        if(Main.a.getServer().getPluginManager().getPlugin("Vault") != null){
            ...
            if(rsp != null){
                ...
                if(Main.eco != null){
                    send.Console(Main.Prefix + " §2Vault / Economy successfully connected!");
                }else{
                    send.Console(Main.Prefix + " §4Economy could not be connected / found!");
                }
            }else{
                send.Console(Main.Prefix + " §4Economy could not be connected / found!");
            }
        }else{
            send.Console(Main.Prefix + " §4Vault / Economy could not be connected / found!");
        }
    }
