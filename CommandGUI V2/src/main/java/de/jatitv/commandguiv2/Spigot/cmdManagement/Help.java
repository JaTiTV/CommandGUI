...
public class Help {
    public static void sendHelp(CommandSender sender, String Prefix) {
        ...
        if (sender.hasPermission("commandgui.command")) {
            ...
            send.sender(sender, Prefix + " " + SelectMessages.HelpCgui.replace("[gui]", Replace.replace(gui.GUI_Name)));
            for (String alias : Main.allAliases) {
                if (Main.guiHashMap.get(alias).GUI_Enable || sender.hasPermission("commandgui.bypass")) {
                    send.sender(sender, Prefix + " " + SelectMessages.HelpOpen.replace("[gui]", alias).replace("[guiname]", Replace.replace(Main.guiHashMap.get(alias).GUI_Name)));
                }
            }
